...
impl Euler {
    pub fn sub_string_divisibility() -> u64 {
        let mut result = 0;
        Self::backtrack(&mut result, &mut Vec::with_capacity(10));
        result
    }
...
mod tests {
    ...
    fn test_sub_string_divisibility() {
        assert_eq!(Euler::sub_string_divisibility(), 16_695_334_890);
    }

    #[test]
    fn test_join() {
        assert_eq!(Euler::join(&[4, 0, 6]), 406);
        assert_eq!(Euler::join(&[0, 6, 3]), 63);
        assert_eq!(Euler::join(&[0, 6, 3]), 63);
    }

    #[test]
    fn test_is_substring_divisible() {
        assert!(Euler::is_sub_string_divisible(&[
            1, 4, 0, 6, 3, 5, 7, 2, 8, 9
        ]),);
    }
}
