...
pub fn truncatable_primes(mut n: usize) -> usize {
    ...
    for prime in prime().skip(5) {
        let len = prime.len();
        if truncate_r_to_l(prime / 10) && truncate_l_to_r(prime, u32::try_from(len - 1).unwrap()) {
            sum += prime;
            n -= 1;
            if n == 0 {
                break;
...
mod tests {
    ...
    fn test_truncate_l_to_r() {
        assert!(truncate_l_to_r(3797, 4));
    }

    #[test]
    fn test_truncate_r_to_l() {
        assert!(truncate_r_to_l(3797));
    }

    #[test]
    fn test_truncatable_primes() {
        assert_eq!(truncatable_primes(11), 748_317);
    }
}
