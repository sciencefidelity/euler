...
impl Euler {
    pub fn integer_right_triangles(target: i32) -> i32 {
        ...
        for i in 1..=target {
            let count = Self::find_count_int_right_triangles(i);
            if count > max_count {
                max_count = count;
                result = i;
            }
    ...
    fn find_count_int_right_triangles(p: i32) -> i32 {
        ...
        for b in 1..=max_ab {
            for a in 1..=b {
                ...
                for c in b..=c_limit {
                    if a + b + c == p && Self::is_pythagorean_triplet((a, b, c)) {
                        count += 1;
                    }
                }
            }
...
mod tests {
    ...
    fn test_integer_right_triangles() {
        assert_eq!(Euler::integer_right_triangles(1000), 840);
    }

    #[test]
    fn test_count_int_rignt_triangles() {
        assert_eq!(Euler::find_count_int_right_triangles(120), 3);
    }
}
